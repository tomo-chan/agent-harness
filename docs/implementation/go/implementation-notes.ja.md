...
`b9e9471a5817a1e1f6cadad297154cf9b71ae381` にある Python の起動処理、ポリシーエンジン、
固定アダプター、信頼済みリポジトリ入力、テスト、S1 レビュー文書である。`97bb264` は
Go 試作着手時の歴史的な基線としてだけ扱い、現行 Python S1 の代表とはしない。
Python の構造や出力を仕様の正本とはせず、対応する保証、実装、テストデータ、実行結果を
比較する。既存 Python 実装は変更しない。

全体仕様の Q-01〜Q-03、Q-10、Q-11 は未解決である。本試作の CLI、スキーマ、
...
| 全体仕様 | Go での具体化 | 根拠 | 状態 |
|---|---|---|---|
| TR-01 | 実行ファイルの正規化済み親を信頼基点と照合し、同じ信頼基点の固定名 `policy.json` だけを読む。アダプターは静的リンクする | `TestTrustedPaths`、`TestSingleBinary` | 配備前提付き適合 |
| IO-01 | 標準入力から単一 JSON を読み、標準出力には判断 JSON だけを出す。正常判断と処理失敗を終了コードで区別する | `TestSingleBinary`、`TestOutputFailure` | 実験スキーマで適合 |
| PO-01 | JSON の記述順によらず `deny > ask > allow`。分類不能時は既定 `ask`、検証不能時は `deny` | `TestPriorityAndPredicates`、`TestInvalidPolicies` | 対象範囲で適合 |
| PO-02 | 方針の全バイトと正規化したツール / 入力の SHA-256、実験的評価器 ID を判断の根拠に含める。外部状態は参照しない | `TestDecisionEvidenceIdentifiesPolicyAndNormalizedAction` | ビルド来歴未確立のため部分適合 |
| FA-01 | パス・設定・方針・入力の失敗時は許可を返さず、拒否 JSON と終了コード 2 を返す。出力不能時も非ゼロ終了する | 異常系のポリシー / フック / 子プロセステスト | 呼出側前提付き適合 |
| CP-01 | 現行 Python S1 とツールポリシーのコマンド検査データ、信頼境界、配備前提を比較し、バイト単位互換や S1 全体の機能同等性を主張しない | `TestPythonS1Vectors`、`TestSingleBinary`、第5・6章 | ツールポリシー中核の比較範囲で部分適合 |
...
現行 Python S1 はツールポリシーの信頼境界だけでなく、S2 が権威として利用する
リポジトリセキュリティポリシーと期待するリポジトリの生成元・再束縛も S1 の主張に含める。
Go 試作はその下流権威入力を実装していないため、現行 Python S1 全体と同じ保証範囲にはない。
これは S2 の意味論を Go で実装すべきという意味ではなく、S2 へ渡す trusted input の確立を
どの信頼されたコンポーネントが担うかが未解決という意味である。
...
- シンボリックリンク解決後の実行ファイルの親が、解決後の信頼基点と一致しなければ拒否する。
- 方針は信頼基点直下の固定名 `policy.json` とし、解決後も信頼基点内にある通常ファイルだけを読む。
- 信頼基点内を指すシンボリックリンクは許容し、境界外シンボリックリンク、欠落、ディレクトリは拒否する。
- アダプターはバイナリへ静的リンクする。任意アダプターを実行時に読み込まない。
...
現行 Python S1 も汎用アダプターを固定し、ツールポリシーは信頼基点内に限定する。
さらに `AGENT_HARNESS_TRUSTED_REPOSITORY_SECURITY_POLICY` を信頼基点内へ束縛し、
...
これは所有権、マウント、署名、更新、失効を検証しない。検査後の置換競合を防ぐのは
信頼された配置の不変性であり、パス検査自体は OS サンドボックスではない。

### Q-02 — CLI と通信

- 引数とサブコマンドを持たない単発プロセスとする。
- 入力は UTF-8 の単一 JSON オブジェクト、上限 1 MiB、深さ上限 64 とする。
- `tool`/`input` または `tool_name`/`tool_input` を受け付ける。同一項目の別名を
  両方指定した場合、重複キー、未知の最上位キー、後続 JSON、不正 UTF-8、型不正を拒否する。
- ツールは空白だけではない文字列、入力はオブジェクトとする。コマンドは文字列だけを許可する。
  `exec` / `Bash` ではコマンドを必須とし、他のツールでは省略できる。`argv` 配列はアダプターごとに
  実行意味が異なり、空白連結すると引数境界を失うため、共通スキーマが確定するまで拒否する。
- context/session/turn/prompt/cwd/event は受理するが、S1 の判断権威には使わない。
  ツール固有入力の追加フィールドは評価器にとって不透明だが、正規化された操作と根拠には保持する。
...
呼出側は、非ゼロ終了、出力欠落・不正、パニック、シグナル、タイムアウトを拒否として扱い、
承認要求を外部承認へ送る必要がある。標準入力の期限とプロセス資源上限は呼出側の責任である。
本試作はベンダー実行環境の停止挙動を保証しない。
...
単一実行ファイルは実行時の可変点を減らす一方、ソースコミット、Go ツールチェーン、ビルドフラグ、
生成成果物の対応が新しい信頼対象になる。本試作の判断根拠はポリシー、正規化した
ツール / 入力、実験的評価器 ID を識別するが、実行中バイナリのソースコミットとビルド来歴は
識別しない。
CI は `ubuntu-latest` と `macos-latest` でテスト / ビルドを行うだけで、配布成果物の
OS / アーキテクチャ行列、再現可能ビルド、署名、SBOM、保管、段階更新、ロールバック、失効、
古いバイナリの検出を確立しない。これらがない状態では、正しい `policy.json` と古いバイナリの
組合せや、未承認ツールチェーンで作られたバイナリを根拠から区別できない。
...
`97bb264` は歴史的な基線であり、現在の選択判断は PR #6 の
...
| 観点 | 現行 Python S1 (`b9e9471`) | Go 試作 | 比較評価 |
|---|---|---|---|
...
| 除去できる失敗形態 | `-I` と環境無害化で子プロセスのユーザーサイト / Python 環境の影響を抑制 | インタープリターの欠落・バージョン差、モジュール読み込み、複数ソースの欠落、アダプターへの実行引継ぎを不要にする | 現行 Python で既に除去した実行時アダプター差替えまで Go 固有の利点として数えない |
| 残存・追加する失敗形態 | インタープリター選択、初回起動、隔離読み込み、ソース組合せ、実行失敗 | ソースとバイナリの対応、ツールチェーン / ビルド来歴、古いバイナリ、OS / アーキテクチャ不一致、署名・更新・ロールバック・失効、RE2 差 | 配置改変、ホスト侵害、TOCTOU、呼出側のフェイルオープンは共通 |
| テスト容易性 / 根拠 | 単体 / 起動処理テスト 22件は成功。ただし隔離引継ぎの正常系 E2E がなく、実起動は読み込みエラー | `go test ./...`、実バイナリの子プロセス、Linux / macOS CI | 現時点の E2E 根拠は Go が強い。配備根拠は双方に不足 |
...
インタープリター、モジュール読み込み、複数ソース、アダプターへの実行引継ぎに起因する失敗形態を除去できる。
...
- 現行 Python S1 が確立する repository-security policy / expected repository の権威を、
  Go または別の信頼されたコンポーネントから S2 へ渡す契約と根拠。
