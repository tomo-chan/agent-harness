...
生成 artifact の対応が新しい信頼対象になる。本 spike の判断 Evidence は policy と action を
識別するが、実行中 binary の version、source commit、build provenance は識別しない。
...
| 判断と再現根拠の結合 | `TestDecisionEvidenceIdentifiesPolicyAndNormalizedAction` | Go にのみ policy/action digest あり |
...
| 入出力 / Evidence | permissive な JSON 正規化、decision/reason/rule | サイズ・深さ・重複・別名・型を厳格検証、policy/action digest | Go の方が protocol ambiguity と再現対象の識別を強く扱う |
