...
このリポジトリには Claude Code、OpenAI Codex、Devin CLI 向けの参照用フック接続を実装している。3製品とも、共通の拒否優先方針エンジン、リポジトリ保護状態検査器、SCM意味論検証器、公開時の制御機構差分審査、決定的な完了保証を利用する。
...
SessionStartでは、`origin` と信頼済み期待リポジトリを比較し、GitHubのリポジトリ情報と既定ブランチに適用される有効な規則を取得する。根拠を `pass` / `fail` / `unknown` に正規化し、`READY` / `RESTRICTED` / `BLOCKED` を導出する。ここで保存するセッションキャッシュは文脈提示と性能最適化のための情報にすぎず、変更操作の権威判断には使用しない。変更操作の実行前には、現在のGit状態とGitHub側根拠から保護状態を再評価する。
...
`.agent-harness/`、ベンダー用フック設定、CI、ハーネス、保護状態検査、方針、起動処理、保証用スクリプト、配備参照、`AGENTS.md` などの保護対象が変更されている場合、規則 `control-plane-publication` の承認要求を返す。
...
SessionStartでは権威を持つ完了基準をローカル状態領域へ保存せず、ローカルの `refs/remotes/origin/*` も遠隔権威の根拠として扱わない。Stop時にリポジトリ保護状態と現在のGit状態を新たに評価し、確認済みGitHubリポジトリから確認済み既定ブランチの現在の先頭コミットSHAを直接取得する。

現在ブランチが確認済み既定ブランチであり、未追跡ファイルを含めて作業ツリーがクリーンで、ローカル `HEAD` がGitHubから直接取得した既定ブランチSHAと完全に一致する場合だけ、ローカルに未完了のリポジトリ成果物がない根拠として成果物向け完了検査を省略する。

機能ブランチ、汚れた作業ツリー、GitHub側既定ブランチと乖離した状態、`BLOCKED`、または確認不能な状態では、通常の決定的な成果物向け完了検査を実行する。

これにより、既定ブランチ上のレビューや調査だけのセッションを誤って失敗させず、同時にリポジトリコードと同じ利用者権限で書き換え可能なセッション基準や遠隔追跡参照を保証根拠として信頼しない。この主張はリポジトリ成果物の状態に限定し、外部システムへの副作用がなかったことまでは保証しない。詳細はDL-020を参照する。
...
回帰検査では、信頼済みルートの隔離、信頼済み最低基準とリポジトリ固有方針の単調合成、信頼済み起動処理によるモード指定、変更操作前の保護状態再評価、複合シェル内の遠隔SCM変更検出、正規形公開、各種強制push、制御機構の公開時審査、プルリクエスト作成時のGit状態拘束、権威状態と承認の優先順位、ローカル遠隔追跡参照を完了保証の権威として使用しないこと、GitHubから直接取得した既定ブランチSHAによる読み取り専用完了、本番Pythonコードの `docstring` を確認する。
