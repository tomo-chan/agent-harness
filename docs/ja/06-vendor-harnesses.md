...
SessionStartでは、権威を持つ完了基準をローカル状態領域へ保存しない。Stop時にリポジトリ保護状態と現在のGit状態を新たに評価する。

現在ブランチが確認済み既定ブランチであり、未追跡ファイルを含めて作業ツリーがクリーンで、ローカル `HEAD` が `origin/<確認済み既定ブランチ>` と完全に一致する場合だけ、ローカルに未完了のリポジトリ成果物がない根拠として成果物向け完了検査を省略する。

機能ブランチ、汚れた作業ツリー、遠隔既定ブランチと乖離した状態、`BLOCKED`、または確認不能な状態では、通常の決定的な成果物向け完了検査を実行する。

これにより、既定ブランチ上のレビューや調査だけのセッションを誤って失敗させず、同時にリポジトリコードと同じ利用者権限で書き換え可能なセッション基準を保証根拠として信頼しない。この主張はリポジトリ成果物の状態に限定し、外部システムへの副作用がなかったことまでは保証しない。詳細はDL-020を参照する。
...
回帰検査では、信頼済みルートの隔離、信頼済み最低基準とリポジトリ固有方針の単調合成、信頼済み起動処理によるモード指定、変更操作前の保護状態再評価、複合シェル内の遠隔SCM変更検出、正規形公開、各種強制push、制御機構の公開時審査、プルリクエスト作成時のGit状態拘束、権威状態と承認の優先順位、現在の権威的な状態による読み取り専用完了、本番Pythonコードの `docstring` を確認する。
