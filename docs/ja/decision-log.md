# 実装判断記録

[English](../decision-log.md) | [ベンダー別ハーネス実装](06-vendor-harnesses.md) | [README](../../README.ja.md)

ハーネスの設計・実装に影響する判断を記録する。詳細な判断記録は [`docs/ja/decisions/`](decisions/) 配下に置く。

## 状態の表記

- **採用** — 現在の設計判断
- **暫定** — 現在のベンダーまたは実行環境の制約に対する一時的な対応
- **廃止** — 履歴として残すが現在は採用しない
- **ベンダー更新時に再評価** — 上流仕様が変わった場合に再検証する

## DL-001 — ベンダー非依存の中央方針エンジン

- 状態: 採用
- 判断: ベンダーごとのフック入力を共通の操作モデルへ正規化し、1つの拒否優先方針エンジンで評価する。

## DL-002 — 方針の優先順位は拒否 > 承認要求 > 許可

- 状態: 採用
- 判断: 記述順に依存せず、拒否規則を最優先する。

## DL-003 — 方針評価エラーは安全側に倒す

- 状態: 採用
- 判断: 不正な方針、壊れた入力、アダプター評価エラーはアダプター境界で拒否または停止とする。ベンダー実行環境そのもののフック障害は、下位のセキュリティ層でも補完する。

## DL-004 — Codexの `ask` はネイティブな強制が安定するまで拒否へ変換

- 状態: 暫定 / ベンダー更新時に再評価
- 判断: 信頼された外部承認がない中央方針の `ask` はCodexでは拒否へ変換する。

## DL-005 — Claude CodeはPreToolUseのネイティブ承認要求を使用

- 状態: 採用 / ベンダー更新時に再評価
- 判断: 中央方針の許可・承認要求・拒否を、Claude CodeのPreToolUse判定へ対応付ける。

## DL-006 — Devinの中央 `ask` は安全側に倒す

- 状態: 暫定 / ベンダー更新時に再評価
- 判断: 外部承認がない中央方針の `ask` は停止扱いとする。Devin固有の権限機構は別の防御層として扱う。

## DL-007 — 現在のワークツリーを動的に解決

- 状態: DL-015により廃止
- 過去の判断: 絶対パスを埋め込まず、`git rev-parse --show-toplevel` でリポジトリルートを解決する。
- 廃止理由: エージェントが変更可能なワークツリーから方針・保証コードを実行すると、信頼済みハーネス境界に違反する。本番環境のフック実行は `AGENT_HARNESS_TRUSTED_ROOT` から解決し、リポジトリ / ワークツリーの検出結果は実行時入力としてのみ扱う。

## DL-008 — ハーネスと設定ファイルはセキュリティ上重要

- 状態: 採用 / DL-019で具体化を強化
- 判断: 制御機構への変更は公開前に明示審査する。編集時のパス検査は多層防御とし、編集経路に依存しない公開時差分検査を主要な判定点とする。

## DL-009 — Stopフックは決定的な完了判定を使う

- 状態: 採用 / DL-020で具体化を強化
- 判断: Stopフックでは決定的な完了保証を使う。確認済み既定ブランチがクリーンで確認済み遠隔既定ブランチと一致する場合だけ読み取り専用のリポジトリ状態として完了でき、それ以外または確認不能な状態では成果物向け完了検査を実行する。

## DL-010 — フックは多層防御であり最終権威ではない

- 状態: 採用
- 判断: 保護ブランチ、認証情報、本番環境、ホストやネットワークの境界は、サンドボックス、ワークロード隔離、IAM / SCM、サーバー側規則でも独立して保護する。

## DL-011 — サンドボックス優先の認証情報露出低減

詳細: [DL-011](decisions/DL-011-sandbox-first-credential-isolation.md)

- 状態: 採用 / ベンダー更新時に再評価
- 判断: サンドボックスとローカル方針で認証情報の露出を減らすが、認証情報の侵害自体は起こり得る故障形態とする。標準構成は **1 Pod / 1エージェントコンテナ** とし、認証情報を隠すことだけを目的にSCM仲介サービスやサイドカーを追加しない。

## DL-012 — SessionStartでリポジトリ保護状態を検査

詳細: [DL-012](decisions/DL-012-sessionstart-repository-posture.md)

- 状態: 採用 / DL-018で具体化を強化 / ベンダー更新時に再評価
- 判断: SessionStartで早期文脈のためにリポジトリ保護状態を検査するが、変更操作の権威判断では書込み可能なセッションキャッシュを信頼せず、保護状態を再評価する。
- 信頼済み識別情報: `AGENT_HARNESS_EXPECTED_REPOSITORY` は信頼済み起動処理が設定する。未設定は `UNKNOWN`、不一致は `BLOCKED`。
- 有効方針: 信頼済み最低基準とリポジトリ固有方針を単調に合成し、リポジトリ側は信頼済み要件を弱められない。
- 信頼済みモード: `AGENT_HARNESS_MINIMUM_POSTURE_MODE` により、信頼済み起動処理は対話用途などで最低基準モードを明示できる。
- 不正な信頼済み方針またはリポジトリ方針は `BLOCKED` とする。
- 強制の分担: SessionStartは早期検出、PreToolUseは変更操作前に再評価して強制し、GitHub側規則とIAMは権威的な外部強制を担う。

## DL-013 — 正規形のSCM公開コマンド

詳細: [DL-013](decisions/DL-013-canonical-scm-publication.md)

- 状態: 採用 / ベンダー更新時に再評価
- 判断: エージェントが直接発行する自律的なGit公開は `git push` と `git push --set-upstream origin HEAD` の2形式だけを許可し、その後にリポジトリ、ブランチ、上流ブランチを意味論的に検証する。
- 複合シェル: 先頭が読み取り専用コマンドでも複合シェルは自律許可対象外とし、`RESTRICTED` ではコマンド途中の遠隔SCM変更も保守的に検出する。
- プルリクエスト作成: 直接の `gh pr create` では、リポジトリ、作業元ブランチ、基準ブランチの上書きを禁止する。

## DL-015 — 信頼済みハーネス境界

詳細: [DL-015](decisions/DL-015-trusted-harness-boundary.md)

- 状態: 採用 / ベンダーまたは実行環境の変更時に再評価
- 判断: 本番環境の方針、保護状態評価器、SCM意味論検証、完了判定コードは、エージェント可変ワークスペースの外に配置された `AGENT_HARNESS_TRUSTED_ROOT` から実行する。
- 基準構成: 承認済みハーネス一式を `/opt/agent-harness` に組み込み、コンテナの読み取り専用ルートファイルシステム上に置く。`/workspace` は可変のまま維持する。
- 方針の出所: 意味論的方針と信頼済み保護状態最低基準は信頼済み領域に置き、リポジトリ固有の保護状態方針はDL-018に従う未信頼の強化入力とする。

## DL-016 — 意味論的方針は完全仲介ではない

詳細: [DL-016](decisions/DL-016-semantic-policy-is-not-complete-mediation.md)

- 状態: 採用 / ベンダーまたは実行環境の変更時に再評価
- 判断: 方針とフックが統治するのは、フック境界で観測可能なエージェント直接操作であり、許可済みコマンドが内部で起動する任意の子プロセスや副作用まで完全仲介するとは主張しない。

## DL-017 — 権威状態を承認より優先する

詳細: [DL-017](decisions/DL-017-authority-state-precedes-approval.md)

- 状態: 採用
- 判断: リポジトリ保護状態は権威状態であり、変更操作について通常の許可・承認要求より先に評価する。
- `BLOCKED`: ネイティブ承認や信頼済み外部承認が存在しても変更を拒否する。
- `RESTRICTED`: ローカル開発は継続できるが、遠隔SCM変更は保護状態が解消されるまで拒否する。

## DL-018 — 信頼済み方針とリポジトリ方針を単調に合成する

詳細: [DL-018](decisions/DL-018-monotonic-posture-policy-composition.md)

- 状態: 採用
- 判断: 信頼済み最低基準を確定した後、リポジトリ固有方針を単調に合成する。モードはより厳しい値、必須要件は論理和、キャッシュ時間はより短い値を採用する。
- 権威: リポジトリ固有方針は制約を追加できるが、信頼済み制約を削除できない。

## DL-019 — 制御機構の変更を公開時に審査する

詳細: [DL-019](decisions/DL-019-control-plane-publication-review.md)

- 状態: 採用
- 判断: 正規形のpushまたは自律的なプルリクエスト作成の前に、遠隔既定ブランチとの差分を検査し、保護対象の制御機構が変更されていれば明示承認を要求する。
- 理由: すべての編集手段が変更対象パスを提示することに依存せず、公開される差分そのものを根拠に審査する。

## DL-020 — 現在の権威的なリポジトリ状態による完了保証

詳細: [DL-020](decisions/DL-020-session-aware-completion-assurance.md)

- 状態: 採用
- 判断: 権威を持つセッション開始時基準は保存しない。停止時に保護状態とGit状態を新たに確立し、クリーンな確認済み既定ブランチが `origin/<既定ブランチ>` と完全に一致する場合だけ、読み取り専用のリポジトリ状態として成果物向け完了検査を省略する。
- 失敗時の扱い: それ以外または確認不能な状態では通常の成果物向け完了検査を実行する。

## 保守規則

設計上の選択肢を決定した場合、ベンダー制約に対する回避策を導入した場合、信頼境界・故障形態・承認経路・セキュリティ不変条件を変更した場合、または将来の更新で単純化できる可能性がある判断を行った場合は、この判断記録を更新する。

暫定判断には更新方針または再評価条件を残す。過去の判断を黙って削除せず、廃止された判断は履歴として残す。
