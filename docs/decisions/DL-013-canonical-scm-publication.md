...
`gh pr create` may not override repository, head branch, or base branch. Repository/head/base are derived from the checked repository and current Git state rather than supplied by the agent.
...
- SCM semantic validator: verifies repository, branch, upstream, and prohibited overrides.
...
Re-evaluate when a vendor exposes structured argv/tool semantics that eliminate shell-string parsing, or provides a first-class SCM publication tool whose repository/ref constraints can be expressed directly. Prefer replacing this validator with the stronger structured primitive rather than expanding shell parsing complexity.
