...
本仕様では、次の意味で用語を使用する。製品固有の名称と同じ語がある場合も、本表の意味を共通契約上の基準とする。
...
| 具体実装 | Concrete Implementation / Realization | 本仕様を Go、Python、設定、配備、外部制御などで実現したもの。仕様そのものではない |
...
### 主体と責務境界（Actors and Responsibility Boundaries）
