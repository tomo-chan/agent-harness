...
利用者・運用者はタスク、許可範囲、完了条件を定める。エージェントは調査・計画・実装・未知の問題の探索を担う。Agent Harness は決定可能な規則の評価と結果の記録を担い、OS / IAM / SCM 等が独立した強制境界を提供する。
...
| 運用者 / 承認ゲートウェイ | 信頼された設定と権限委譲の管理 | リポジトリ管理下の指示と独立する |
| 外部強制機構 | サンドボックス、ワークロード分離、ネットワーク、IAM、SCM の強制 | フックやエージェントの協力に依存しない |
...
**詳細化項目:** リポジトリ識別情報の正本、リポジトリ状態の取得元・鮮度、ワークツリーとブランチの対応、シンボリックリンクや別リモート、検証と実行の間の状態変化（TOCTOU）を扱う（Q-04）。外部 SCM の保護をローカル判定だけで保証したことにしない。
...
**詳細化項目:** 制御プレーン分類と管理主体、push の参照仕様 / 宛先、PR の変更元 / 変更先、差分の確定方法、認可・根拠の有効範囲を定義する（Q-05）。SCM のサーバー側認可を最終権威とし、ローカル許可による迂回を認めない。
...
| 外部 API / SCM / ネットワークエラー | 成功と断定しない。外部状態を照合してから再試行する |
...
| SCM 規則 / ブランチ保護と迂回不能な認証情報 | SCM 管理者 | 保護ブランチの最終防御を主張できない |
...
- サンドボックス、コンテナ、IAM、SCM のサーバー側ポリシーの置換。
...
| S3 SCM 公開 | PU-01、RE-01/02、PO-01/02 |
...
- 外部 IAM / SCM / サンドボックスの根拠。単体テストで外部保護の成立を代用しない。
...
| Q-09 | エラー分類、部分成功、監査失敗と秘匿化 | 失敗 / 監査契約。関連する外部操作・記録の具体化前 |
| Q-10 | 仕様・スキーマのバージョン、互換性と移行 | 互換性方針と実装差分表。安定版契約の公開前 |
| Q-11 | テストデータ、追跡可能性、必須 CI、部分適合表示 | テスト / 保証契約。各範囲の適合宣言前 |

[Issue #14](https://github.com/tomo-chan/agent-harness/issues/14) の Go による試作は、全体仕様の信頼された実行環境 / ポリシー強制を Go で具体化し、S1 保証スライスで評価する作業として位置づける。Go の配布・内部構造・Python との差分は実装ノートで扱い、本仕様の名前や章立てを Go / S1 専用へ戻さない。
