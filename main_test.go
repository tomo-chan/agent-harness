...
func TestSingleBinary(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "ask", config: valid, input: `{"tool":"exec","input":{"command":"unknown"}}`, want: "ask"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd.Dir = cwd
			cmd.Env = []string{"PATH=/nonexistent", "PYTHONPATH=" + cwd, "PYTHONHOME=" + cwd, "AGENT_POLICY=" + evil, "AGENT_HARNESS_POLICY=" + evil}
			if !tc.noRoot {
