...
func (g fakeGit) Run(_ context.Context, _ string, args ...string) (string, error) {
	k := strings.Join(args, " ")
	if err := g.failures[k]; err != nil {
		return "", err
	}
	if value, ok := g.values[k]; ok {
		return value, nil
	}
...
func (g fakeGate) Check(context.Context, string) (string, error) {
	if g.called != nil {
		*g.called = true
	}
...
func report(branch string) repository.Report {
	return repository.Report{State: "READY", Repository: "acme/widget", RepoRoot: "/repo", Branch: branch, HeadSHA: strings.Repeat("a", 40), DefaultBranch: "main"}
}
...
func TestCleanDefaultBranchRequiresAgentReviewFirst(t *testing.T) {
	...
	r := Evaluate(context.Background(), Request{CWD: "/repo"}, report("main"), g, fakeGitHub{head: head}, fakeGate{called: &gateCalled, detail: "task predicates passed"})
	if r.Outcome != OutcomeReviewRequired || !gateCalled || !r.Evidence.NoDeliveryDelta {
		t.Fatalf("%+v gate=%t", r, gateCalled)
	}
}
...
func TestFeatureBranchUsesDeterministicGate(t *testing.T) {
	...
	if r.Outcome != OutcomeReviewRequired || !called || r.Evidence.NoDeliveryDelta {
		t.Fatalf("%+v called=%t", r, called)
	}
}
...
func TestCompletionRejectsDifferentWorktreeWithSameBranchAndHead(t *testing.T) {
	head := strings.Repeat("a", 40)
	g := fakeGit{values: map[string]string{"rev-parse --show-toplevel": "/other", "rev-parse --abbrev-ref HEAD": "feature/task", "rev-parse HEAD": head}}
	r := Evaluate(context.Background(), Request{CWD: "/other"}, report("feature/task"), g, fakeGitHub{}, fakeGate{})
	if r.Outcome != OutcomeBlocked || !strings.Contains(r.Reason, "worktree") {
		t.Fatalf("%+v", r)
	}
}
