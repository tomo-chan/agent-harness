"""Connect vendor lifecycle adapters to the shared S1-S5 guarantee implementation.
...
evaluation, non-authoritative SessionStart context, and Stop-time completion
assurance. Stop payloads are passed through unchanged so S5 can use vendor
continuation evidence such as ``stop_hook_active`` without vendor-specific state.
"""
...
def completion(raw: dict[str, Any]) -> tuple[bool, str]:
    """Run S5 Stop-time assurance while preserving the original vendor payload."""
